...
public class DepartmentService {
    ...
    public DepartmentService(DepartmentRepository repository, DepartmentMapping departmentMapping, DeveloperMapping developerMapping) {
    ...
    public List<Department> saveDepartments(List<DepartmentDTO> departments) {
        return repository.saveAll(departments.stream().map(departmentMapping::mapToDepartmentEntity).collect(Collectors.toSet()));
    }
    ...
    public Department updateDepartment (DepartmentDTO department) {
        Department existingDepartment = repository.findById(departmentMapping.mapToDepartmentEntity(department).getId()).orElse(null);
        existingDepartment.setName(department.getName());
        existingDepartment.setDescription(department.getDescription());
        existingDepartment.setDevelopers(department.getDeveloperDTOList().stream().map(developerMapping::mapToDeveloperEntity).collect(Collectors.toSet()));
